...
To infinity and beyond! Clone the repo, enable Yarn 4 via Corepack, then install root dependencies:
...
This project requires **Yarn 4.5.0** (see `packageManager` in `package.json`). Classic Yarn 1.x fails at setup with `Couldn't find the node_modules state file` or a `packageManager` mismatch.
